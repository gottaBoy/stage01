				...
				<div class="issue swiper-wrapper " id="issue">
					<div class="cnt swiper-slide swiper-slide-visible swiper-slide-active artist">
				 		<ul class="list">
				 			...
				 			<s:if test="#request.step1Name != ''">
				 				value="<s:property value="#request.step1Name"/>" readonly="readonly"
				 			...
				 			<s:if test="#request.step1Mobile != ''">
				 				value="<s:property value="#request.step1Mobile"/>" readonly="readonly"
				 			</s:if>
				 			 class="artname ctel"> *</div></li>
				 			<li><div class="inp1"><label>Email：</label><input type="text"
				 			<s:if test="#request.step1Email != ''">
				 				value="<s:property value="#request.step1Email"/>" readonly="readonly"
				 			</s:if>
				 			 class="artname cmail"> <font style="color:orange">*</font></div></li>
				 		...
				 		<div id="step2-uploadedImg" style="display:none;"></div>
				 		<div class="tcenter mb10">
				 			<a href="javascript:void(0);" id="art-reg-commit" style="background-color: #308ACA; width:50%; margin: 0 auto; border-radius: 5px; height: 35px; color:#fff; font-size:14px; text-align: center; line-height:35px; cursor: pointer">完成</a>
				 		</div>
					</div>		
				</div>
			</div>
  ...
  $("#testmm").localResizeIMG({
    ...
    before: function(that, blob) {
      ...
      if (blob) {
        ...
        //console.log(g); // 文件名称
        //console.info(g);
        $("#step2-uploadedImg").data("imagename", g);
      } else {
    ...
    success: function(result) {
      ...
      $("#step2-uploadedImg").data("imagedata", imagedata);
...
<script>
	...
	$("#art-reg-commit").click(stepRegCommit);

	function stepRegCommit() {
		...
		//pic
		var picName = $("#step2-uploadedImg").data("imagename");
		var picdata = $("#step2-uploadedImg").data("imagedata");
		...
		*/
		...
		$("#art-reg-commit").off("click");
		$("#art-reg-commit").html("处理中...");
		mui.toast("正在处理请求...");

		$.ajax({
			url : "weih5/doArtRegisterStep2.htm",
			type : 'POST',
			dataType : 'json',
			data : param,

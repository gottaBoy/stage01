...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<!--    org/orgReal-name.jsp  <%=request.getParameter("userId")%>   -->
		...
		<link rel="stylesheet" type="text/css"
			href="<%=request.getContextPath() %>/theme/css2/orgzhezhao.css" />
	...
	<script type="text/javascript" language="javascript" src="<%=request.getContextPath() %>/theme/js/swfobject.js"></script>
		...
		<script type="text/javascript" src="<%=request.getContextPath() %>/theme/js2/welcome.js"></script>
		<script src="<%=request.getContextPath() %>/theme/js2/slides.min.jquery.js"></script>
...
<script type="text/javascript">
	...
	function doSubmit1(){
	    ...
	    if($("#userInfo\\.orgCard").val()==""){
			art.dialog.alert('请输入营业证号码');	    
			$("#userInfo\\.orgCard").focus();
			return;
	    }
		if(!$("#imgFile3").attr("src")){
				if(!checkFileName("myFile3","营业执照")){ 
					return;
				}
		}
...
function clearAction(){
	...
	<body>
		<!-- 主内容开始 -->
		<div class="zhezhao" style="height: 710px;">

			<div class="shiming">
				<h3>
					企业认证<font style="color:gray;font-size:12px">&nbsp;&nbsp;(请上传企业营业执照；若无，请上传企业负责人身份证。)</font>
				</h3>
...
<input type="hidden" name="type" value="org">
				...
				<div class="boxRight">
					<ul class="lc">
						...
						<div class="box tab2Content" style="border:solid 1px white; display:block">
							<ul class="cert-group clearfix">
								<li class="cert">

									<ul id="ptrz" class="cert-list">
										<li class="titlee">
											营业执照认证
										</li>
										<li>
											营业执照号码（必须）与提交的营业执照号码一致
										</li>
										<li>
											<div class="shiming2">
												上传营业执照
												<span>(必传)</span>
												<c:if test="${userInfo.orgCardCopy != null}">
														<img onerror="handleImgLoadError(event);"   id="imgFile3" onclick="view(this)" src="/stageUpload/idCard/<s:property value="#request.userInfo.orgCardCopy"/>?id=<%=new Date() %>" style="max-width: 50px;max-height: 30px;"/>
												</c:if>
											</div>

											<div class="shiming3">
												<input type="file" name="myFile3" value="" id="myFile3" style="width: 288px; height: 27px; outline: none;" onchange="checkFileName('myFile3','营业执照')"><br>
											</div>

											<div class="shiming4">
												营业执照号码
												<span>(必填且必须与提交的营业执照号码一致)</span>
											</div>

											<div class="shiming5">
												<input name="userInfo.orgCard" id="userInfo.orgCard" value="${userInfo.orgCard}"/>
											</div>
										</li>
										<li>
											<button type="button" class="cert-btn" onclick="doSubmit1()">
												立即认证
											</button>
										</li>
									</ul>
						...
						<div class="box tab2Content"  style="border:solid 1px white;display:none">
							<div class="renzheng" >
...
<c:choose>
<c:when test="${userInfo.idCardCopy2 == null}">
  <tr>
...
<c:otherwise>
  <tr>
    ...
    <td>
     ...
     <div class="shiming6" style="width:100%;height:40px;margin:0px; margin-left:150px;">
            <div style="float:left;width:200px"><a href="javascript:doSubmit();"  id="saveBtn">确认保存</a></div>
            <div style="float:left;width:200px"><a href="javascript:clearAction();">关闭</a></div>
     </div>
				...
				<div class="shiming7" style="float:left;clear:both;">
					<h6>
						特别注意
					</h6>
					<img src="<%=request.getContextPath() %>/theme/images/small_star.jpg" width="10" height="10">
						（身份证/护照/军官证均可）用相机拍下来，照片清晰无遮挡。
					<br />
					<img src="<%=request.getContextPath() %>/theme/images/small_star.jpg" width="10" height="10">
						&nbsp;您的证件仅在审核人员审核时可以看见，第1舞台严格保护您的隐私，请放心。
					<br />
					<img src="<%=request.getContextPath() %>/theme/images/small_star.jpg" width="10" height="10">
						&nbsp;身份证相片必须与个人相片吻合，否则视无效申请！ 
				</div>
</form>
			</div>
...
<script language="javascript">
		<% if(saveSuccess!=null){%>
			art.dialog.confirm("提交成功，等待审核！",function () {    art.dialog.close();   },    function () {    art.dialog.close();  });
		<%}%>
</script>

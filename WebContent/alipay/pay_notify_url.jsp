		...
		try {
			...
			if(success_details != null) {
				...
				for(int i = 0; i < orderArray.length; i++) {
					...
					String orderCode = orderNo.substring(0, orderNo.length() - 3);//订单编号
					String posStr = orderNo.substring(orderNo.length() - 3, orderNo.length());
					int pos = Integer.parseInt(posStr);//付款序号
					...
					while(rs.next()) {
						if(rs.getString("status").equals("1")) {
							...
							stmt1.executeUpdate("UPDATE t_app_business_pay SET isPay=2,payTime='"+createDateTime+"' WHERE pos="+pos+" and orderid=(select id from t_app_business_info where orderCode='"+orderCode+"')");
							
							
							ResultSet rs1 = stmt2.executeQuery("select COUNT(*) a from t_app_business_pay where isPay=1 and orderId=(select id from t_app_business_info where orderCode='"+orderNo+"')"); 
							int count = 0;
							while(rs1.next()) {
								count = rs1.getInt("a");
							}
							if(count == 0) {
								stmt1.executeUpdate("UPDATE t_app_business_info SET status=10,createTime5='"+createDateTime+"' WHERE orderCode='"+orderCode+"'");
								stmt1.executeUpdate("UPDATE t_app_user_info SET SignNumber=SignNumber+1 WHERE userId=(select seller from t_app_business_info where orderCode='"+orderCode+"')");
							}
						}
					}
				}

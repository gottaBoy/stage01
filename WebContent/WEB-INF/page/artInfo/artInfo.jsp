...
function toAddArt(sign){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toAddArtInfo.htm?sign='+sign+'&<%=HttpSessionHelper.getParamData(request)%>',{width:600,height:300,resize:false,lock:true ,close: 
		function () {
			getUserArt(1);
		}});
}
...
function getUserArt(type) {
	$.ajax({
		url:"<%=request.getContextPath() %>/member/userinfo!getUserArtList1.htm?type=" + type+"&<%=HttpSessionHelper.getParamData(request)%>",
		type:'POST',
		dataType:'json',  

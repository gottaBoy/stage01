...
//		}
	...
	_addVocation = function (vocationId) {
		...
		//在当前职业显示的dom
		var html = '<div class="current-jobBox" id="'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'">\
						<a href="javascript:void(0);" title="" hidefocus="true">&nbsp;</a>\
						<input type="hidden"  id="div_'+vocationId+'" /> \
						<input type="hidden" name="profession_'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'" id="profession_'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'"  value="'+vocationId+'"/> \
						<input type="text" name="bprice'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'" id="bprice'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'" maxlength="10" style="width:30px;"  onkeydown="checkKeyForFloat(this.value,event)"  value=""  class="duan" />\
						    <span class="heng">-</span>\
						    <input type="text" name="eprice'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'" id="eprice'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'" maxlength="10" style="width:30px;"  value="" onkeydown="checkKeyForFloat(this.value,event)"  class="duan" />\
						    <span class="heng">/</span>\
						   <input type="text" name="unit'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'"  id="unit'+$(".current-job .current-jobBox .current-null:first").parent().attr("id")+'" maxlength="10" style="width:25px;"  value=""  class="add_popup_input" /><font color=red>(单位)场、次、小时、天等</font>\
						<input type="button" class="btn" value="删除">\
						<input type="checkbox" name="vocations" value="" checked="checked" style="display: none" />\
					</div>';
		var $html = $(html).attr("id", $(".current-job .current-jobBox .current-null:first").parent().attr("id"));
		$html.find("img").attr({
					"title" : description,
					"src" : img_src
				});
	...
	_delVocation = function (boxId) {
		$("#div_"+boxId).parent().html('<div class="current-null">未选择</div>');
	}

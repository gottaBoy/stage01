...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
        ...
        function doSubmit(){
			...
			if(drawbackAmount*1>returnPrice*1){
 			...
 			$.ajax({
			   url:"<%=request.getContextPath() %>/member/drawback!doEditDrawback.htm",
			   type:'POST',
  ...
  <body>
	<form id="drawBackForm" name="drawBackForm" action="member/drawback!doEditDrawback.htm" method="post">
	<input type="hidden" name="drawback.id" value="${drawback.id }"/>
	<input type="hidden" name="drawback.drawbackNo" value="${drawback.drawbackNo }"/>
	<input type="hidden" name="drawback.businessId" value="${drawback.businessId }"/> 
	<input type="hidden" name="drawback.creatTime" value="${drawback.creatTime }"/> 
	<input type="hidden" name="drawback.choucheng" id="choucheng" value="${drawback.choucheng }"/> 
	<input type="hidden" name="drawback.status" value="${drawback.status }"/> 
    ...
    <tr>
        ...
        <table width="460" border="0" align="center" cellpadding="0" cellspacing="0" class="add__Popup_con">
           	<tr>
                <td  align="right" width="100px">交易编号：</td>
                <td  align="left" colspan="3"><span>${businessNo }</span></td>
            </tr>
            <tr>
                 <td  align="right" width="100px">交易金额：</td>
            ...
            <tr>
                ...
                <td  align="left" colspan="3" ><input type="text" name="drawback.drawbackAmount" id="drawbackAmount" value="${drawback.drawbackAmount}"  onkeyup="calPrice()" />
                		<input type="button" name="allBack" id="allBack" value="全额退款" onclick="allBackAmount();"/></td>
             </tr>
            <tr>
                <td  align="right" >实际退款金额：</td>
                <td  align="left" colspan="3" ><span id="price">${drawback.price }</span><span style="color:red;">(扣除手续费<s:property value="#request.choucheng"/>%)</span></td>
             </tr>
		    <tr>
		        <td align="right"><font color="red">*</font>退款原因：</td>
		        <td align="left" colspan="3"><textarea style="width:350px;" name="drawback.drawbackCause" id="drawbackCause" cols="20" rows="4" class="add_popup_textarea ">${drawback.drawbackCause}</textarea></td>
		     </tr>
		     <tr>
		       <td align="right"></td>

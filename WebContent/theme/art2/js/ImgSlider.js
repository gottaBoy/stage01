...
window.onload = function(){
	...
	var i = 0;
	oUl.style.width = oLi.length * (oLi[0].offsetWidth + 8)+'px';
	...
	function td(l){
		...
		oUl.style.left = '-'+(oUl.offsetWidth - (oConter.offsetWidth + 8)) * scrol +'px';
	}
};
//运动框架
...
$(function(){
	...
	$('.pre').click(function(){
		var h2title=$(this).parents('.box-slider').find('.on').prev('li').find('h2').text();
		var ptitle=$(this).parents('.box-slider').find('.on').prev('li').find('p').text();
		$('.intro h2').text(h2title);
		$('.intro p').text(ptitle);
		$(this).parents('.box-slider').find('.on').removeClass('on').prev().addClass('on');
	...
	$('.next').click(function(){
		    var h2title=$(this).parents('.box-slider').find('.on').next('li').find('h2').text();
			var ptitle=$(this).parents('.box-slider').find('.on').next('li').find('p').text();
			$('.intro h2').text(h2title);
			$('.intro p').text(ptitle);
			$(this).parents('.box-slider').find('.on').removeClass('on').next().addClass('on');
			var qsrc=$('.on').find('img').attr('src');
...
$(function() {
	$(".tab-items .tab").click(function() {
		...
		var $this = $(this),
			idx = $(".tab-items .tab").index($this),
			$parent = $this.parents('.tab-items'),
			$panels = $parent.next('.tab-panels').children();
		if(!$panels.eq(idx).hasClass('active')) {

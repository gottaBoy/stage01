...
function getUserArt(type,currentPage,begin,length) {
	$.ajax({
		url:"<%=request.getContextPath() %>/member/userinfo!getUserArtList.htm?currentPage="+currentPage+"&length="+length+"&begin="+begin+"&type="+type+"&<%=HttpSessionHelper.getParamData(request)%>",
		type:'POST',
		dataType:'json',  
...
function toAddArt(sign){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toAddArtInfo.htm?sign='+sign+'&<%=HttpSessionHelper.getParamData(request)%>',{width:750,height:450,resize:false,lock:true,close: 
		function () {
			getUserArt(2,1,0,15); 
		}});
}
 ...
 function toUpdateArt(sign,id){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toUpdateArtInfo.htm?id='+id+'&<%=HttpSessionHelper.getParamData(request)%>&sign='+sign,{width:750,height:450,lock:true,resize: false,close: 
		function () {
			getUserArt(2,1,0,15); 
		}});
}

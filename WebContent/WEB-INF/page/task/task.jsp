...
//------------------------用来计算阴历---END---------------------
  ...
  function calendar(today){
      ...
      content+="<A href='javascript:NowMonth()'><b>本月</b></A> <td class='D_bt'><a href='javaScript:toAddSchedule();' title='添加任务'><img src='<%=request.getContextPath() %>/theme/images/add_nav.jpg' align='absmiddle'></a>"; 
	  ...
	  for   (i=1;   i<=nDays;  i++){   //画出主体部分的日期
		...
		else
		{
			...
			content+="</div>"
               ...
               <%List userTaskList=(List)request.getAttribute("userTaskList");
               for(int i=0;i< userTaskList.size();i++){ 
               	 TAppTask userTask=(TAppTask)userTaskList.get(i);
               	 int id=userTask.getId();
               	 String active=userTask.getActive();
               	 String title=userTask.getTitle();
               	 String titleWrite=title;
               	 if(title.length()>5){
               	 	titleWrite=title.substring(0,5)+"...";
               	 }
               	 String startDateTime=userTask.getStartDate();
               	 String startDate=startDateTime.substring(0,10);
                 String endDateTime=userTask.getEndDate();
              ...
              if(todayDate>=startDateResult&&todayDate<=endDateResult){
   				  content+="<p><a href='javaScript:toUpdateSchedule(<%=id %>);' ";
   				  <% if(active.equals("-1")){ %>
   				  content+="title='查看' class='date_sj'><IMG src='<%=request.getContextPath() %>/theme/images/sj_ico_huise.gif'>"
   				  <% }else{%>
   				  content+="title='查看、修改、结束任务、删除' class='date_sj'><IMG src='<%=request.getContextPath() %>/theme/images/sj_ico.gif'>"
   				  <% }%>
   				  content+="&nbsp;<%=titleWrite %></a>"
 			   }
			   <%
			    }
                %>

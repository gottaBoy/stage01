...
<table width="99%">
	<tr>
		<td><form id="searchFrom" method="post" action="<%=request.getContextPath() %>/member/task.htm">
			<table width="80%">
				<tr>
					<td>
...
<s:iterator value="#request.userTaskList" var="item" status="st">
	<tr >
		...
		<td>
			<s:if test="#item[0].status==2">
				<s:property value="#item[0].Mobile.split(',')[1]" />-<s:property value="#item[0].Mobile.split(', ')[2]" />元
			</s:if>
			<s:else><s:property value="#item[0].price" />元</s:else>
		</td>
		<td><s:if test="#item[0].status==2">无</s:if><s:else>【<a href="<%=request.getContextPath() %>/member/task!toPersonTask.htm?taskId=<s:property value="#item[0].id" />" style="color: #0099ff;" target="_blank"><s:property value="#item[0].personCount" /></a>】人</s:else></td>
		<td>

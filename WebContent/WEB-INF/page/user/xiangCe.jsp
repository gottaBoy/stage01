...
function toAddArt(sign){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toAddArtInfo.htm?sign='+sign,{width:600,height:300,resize:false,lock:true ,close: 
		function () {
			getUserArt(1);
		}});
}
...
function toUpdateArt(sign,id){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toUpdateArtInfo.htm?id='+id+'&<%=HttpSessionHelper.getParamData(request)%>&sign='+sign,{width:600,height:300,lock:true,resize: false,close: 
		function () {
			getUserArt(1);
		}});
}
...
function getUserArt(type) {
	$.ajax({
		url:"<%=request.getContextPath() %>/member/userinfo!getUserArtList1.htm?type=" + type,
		type:'POST',
		dataType:'json',  
		success:function(data){
			...
			if(d.length==0){
				...
				for(var i = 0; i < d.length; i++) {
					...
					if(type == 1) {
						html +="<li><img onclick='openXiangce("+obj.id+")' onerror='handleImgLoadError2(event);' src='"+obj.url+"' width='140' height='140' /><h6 >"+obj.title+"</h6><h6 style='margin:0 auto'>";
						html +="<a href='javascript:toUpdateArt(1,"+obj.id+");' class='bianji'>编辑</a><a href='javascript:toDelArt("+obj.id+",1);' class='bianji'>删除</a></h6></li>"
					}

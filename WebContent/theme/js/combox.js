(function($){
	$.combox={
		load: function(objId,refUrl,selectedValue,label){
			if(refUrl==""){
			...
			return this.each(function(){
				...
				$(op.selector, box).click(function(){
					...
					if (options.is(":hidden")) {
						...
						var top = box.offset().top+box[0].offsetHeight-0;
						if(top + options.height() > $(window).height() - 0) {
							top =  $(window).height() - 0 - options.height();
						}

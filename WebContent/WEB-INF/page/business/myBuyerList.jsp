...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	function viewEvaluate(id){
		art.dialog.open('<%=request.getContextPath() %>/member/evaluate!addEvaluate.htm?id='+id+'&flag=2',{width:550,height:450,resize: false});
	}

	function addYrEvaluate(id){//alert("功能待测试");
		//art.dialog.open('<%=request.getContextPath() %>/member/evaluate!addEvaluate.htm?id='+id+'&flag=0',{width:550,height:350,resize: false});
	}
	function updateYrEvaluate(id){//alert("功能待测试");
		//art.dialog.open('<%=request.getContextPath() %>/member/evaluate!addEvaluate.htm?id='+id+'&flag=1',{width:550,height:350,resize: false});
	}
	...
	function alignAddOrder(id) {
		$.ajax({
		   url:"<%=request.getContextPath() %>/member/business!alignAddOrder.htm?id="+id,
		   type:'POST',
	...
	function search(type) {
		if(type == 4) {
			...
			window.location.href= "<%=request.getContextPath() %>/member/business!myBuyerList.htm?pageNo="+pageNo+"&rowCount="+rowCount+"&type="+type;
		}
	}	
</script>
  ...
  <table width="980" border="0" align="center" cellpadding="0" cellspacing="0" class="layout_table">
    <tr>
        ...
        <s:action name="menuHyzx" executeResult="true">
        	<s:param name="menuFlag" value="5"></s:param>
        </s:action>
       <!-- 会员 左导航 结束 -->
</td>
        <td height="720">&nbsp;</td>
        <td width="810" align="left" valign="top">
        <div class="your_position">您的位置：<a href="#">首页</a> > <a href="#">我的交易</a></div>


<input type="hidden" value="<s:property value='#request.pageNum'/>" id="pageNum" />
...
<div style="border:#f9dfb2 1px solid; background-color:#ffffe0; height:25px; margin-right:15px; padding-top:5px;">
  ...
  <span style="color:#36c;padding-left:10px; cursor:pointer;" onclick="search(1)">待完成任务<span style="color:#c33;">(<s:property value="#request.waitCount"/>)</span></span>
  <span style="color:#36c;padding-left:10px; cursor:pointer;" onclick="search(2)">待确认完成任务<span style="color:#c33;">(<s:property value="#request.confirmCount"/>)</span></span>
  <span style="color:#36c;padding-left:10px; cursor:pointer;" onclick="search(3)">待评价<span style="color:#c33;">(<s:property value="#request.rateCount"/>)</span></span>
...
<div style="border-top:#c3d8e5 1px solid;border-bottom:#c3d8e5 1px solid; height:25px; margin-right:15px; margin-top:5px; height:35px; line-height:35px;">
  ...
  <span style="color:#36c;padding-left:10px;"><input type="button" value="搜索" onclick="search(4)" style="background:url(<%=request.getContextPath() %>/theme/images2/business_bg.png) 0px -52px; width:52px; height:25px;" /></span>
  </form>
</div>
...
<table width="795">
  <tr style="background:url(<%=request.getContextPath() %>/theme/images2/business_bg.png); height:32px;">
    ...
    <td width="80" align="center">应付款</td>
    <td width="80" align="center">已付款</td>
  ...
  <s:iterator value="#request.businessInfoList" var="businessInfo" >
    <tr style="background-color:#e8f2ff; border:#d4e7ff solid 1px;" height="35">
      ...
      <td colspan="9">  
        ...
        <span style="color:#36c;padding-left:10px; cursor:pointer;" onclick="sendMessage(<s:property value='#businessInfo.userInfo.userId'/>)">联系他</span>&nbsp;&nbsp;<span style="color:#36c;padding-left:10px; cursor:pointer;" onclick="userView(<s:property value='#businessInfo.userInfo.userId'/>)"><s:property value="#businessInfo.userInfo.nickName"/></span>
      </td>
    </tr>
    <tr style="border-left:#d4e7ff solid 1px;border-right:#d4e7ff solid 1px;border-bottom:#d4e7ff solid 1px;">
      ...
      <td align="center" valign="top" style="border-right:#e6e6e6 solid 1px;">
          ...
          <s:if test="#businessInfo.business.status==10">
            ...
            <s:elseif test="#businessInfo.business.sellerRate==2"><div class="opDiv">卖家已评价</div>
            </s:elseif>
            <s:elseif test="#businessInfo.business.buyerRate==2"><div class="opDiv">已评价</div>
            </s:elseif>
          </s:if>
      </td>
      <td align="center" valign="top" style="border-right:#e6e6e6 solid 1px;">
          ...
          <s:if test="#businessInfo.business.status==1">
            <div class="opDiv"><a href="javaScript: toBusinessUpdate(<s:property value="#businessInfo.business.seller"/>,<s:property value="#businessInfo.business.id"/>);" class="czjl_title_link">修改订单</a></div>
            <div class="opDiv"><a href="javascript:updateBusinessInfoStatus(<s:property value="#businessInfo.business.id"/>,9);" class="czjl_title_link">取消订单</a></div>
          </s:if>
                                                    
          <s:if test="#businessInfo.business.status==2 || #businessInfo.business.status==3 ">
            <div class="opDiv"><a href="javascript:updateBusinessInfoStatus(<s:property value="#businessInfo.business.id"/>,4);" class="czjl_title_link">确认任务完成</a></div>
            <div class="opDiv"><a href="javascript:updateBusinessInfoStatus(<s:property value="#businessInfo.business.id"/>,9);" class="czjl_title_link">取消订单</a></div>
          </s:if>
                               
          <s:if test="#businessInfo.business.status==10 || #businessInfo.business.status==6 ">
            <s:if test="#businessInfo.business.buyerRate==1"><div class="opDiv"><a href="javascript:addEvaluate(<s:property value="#businessInfo.business.id"/>);" class="czjl_title_link">评价</a></div>
            </s:if>
            <s:else>
				<div class="opDiv"><a href="javascript:viewEvaluate(<s:property value="#businessInfo.business.id"/>);" class="czjl_title_link">已评价</a></div>
              <s:iterator value="#request.infoToEvaluateList" var="infoToEvaluateList" >
                  <s:if test="#infoToEvaluateList.businessId==#businessInfo.business.id"><div class="opDiv"><a href="javascript:updateEvaluate(<s:property value="#businessInfo.business.id"/>);" class="czjl_title_link">修改评价</a></div></s:if>  
              </s:iterator> 
            </s:else>
          </s:if>
      </td>
...
<table width="790" border="0" align="center" cellpadding="0" cellspacing="0" class="page">
    <tr>
        <td align="center"><pg:pages  pageNo='${pageNum}' rowCount ="10"  url="business!myBuyerList.htm" total='${total}' ></pg:pages></td>
    </tr>
</table>

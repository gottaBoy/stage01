 ...
 <s:if test="#talentCategory2.layer==2 && #talentCategory2.parentId==#request.sign">
 	<tr><td>
      <div style="height:30px; line-height: 30px;font-size:14px;">
      	<a style=" color:#FFF;" href="<%=request.getContextPath() %>/query/queryAction!toQuery.htm?signSub=<s:property value="#talentCategory2.id"/>&sign=<s:property value="#request.sign"/>&filterValue=<s:property value="#talentCategory2.alias"/>"><s:property value="#talentCategory2.typeName"/></a>
      </div>
     </td></tr>
     <tr> <td>
      <s:iterator value="#request.list1" var="talentCategory3">
      		<s:if test="#talentCategory3.parentId==#talentCategory2.id">
			   <div style="clear:both"></div>
      			<div style="float:left;font-size:12px; margin-left:10px; line-height: 25px;"><a style=" color:#FFF;" href="<%=request.getContextPath() %>/query/queryAction!toQuery.htm?signSub=<s:property value="#talentCategory3.id"/>&sign=<s:property value="#request.sign"/>&filterValue=<s:property value="#talentCategory3.alias"/>"><s:property value="#talentCategory3.typeName"/></a></div>
					<div style="clear:both"></div>
					  <s:iterator value="#request.list1" var="talentCategory4">
							<s:if test="#talentCategory4.parentId==#talentCategory3.id">
								<div style="float:left;font-size:12px; margin-left:30px; line-height: 25px;"><a style=" color:#FFF;" href="<%=request.getContextPath() %>/query/queryAction!toQuery.htm?signSub=<s:property value="#talentCategory4.id"/>&sign=<s:property value="#request.sign"/>&filterValue=<s:property value="#talentCategory4.alias"/>"><s:property value="#talentCategory4.typeName"/></a></div>
							</s:if>
						</s:iterator>
...
&nbsp;</td>
  ...
  <ul class="ejy-rp2">
      <li class="jj2"><a id="sort_1" href="javascript:submitSearchFrom(2,1)">能力评级</a></li>
...
<%--       <li class="jj2"><a href="<%=request.getContextPath() %>/query/queryAction!toQuery.htm?signSub=<%=signSub %>&sign=<%=sign %>&filterValue=<%=filterValue %>&eprice=${eprice }&bprice=${bprice }&pageKeywords=${keywords }&pageHeadKeywords=${headKeywords }&orderBySign=4&orderByDesc=${orderByDesc==0?1:0}">所在地</a></li> --%>
  ...
  <tr class="ejy-rp6" valign="middle">
    <td>
      <dl class="ejy-dl">
         ...
         <dd>能力：
				 <s:if test="#item[2].keywords==''">
					<s:property value="#item[3].typeName" />
				 </s:if>
				 <s:else>
					 <s:if test="#item[2].keywords.length() > 20">
						<s:property value="#item[2].keywords.substring(0,20)" />...
					 </s:if>
					<s:else>
						<s:property value="#item[2].keywords" />
					</s:else>				 
				 </s:else>
		</dd>
  ...
  <tr>
    ...
    <table width="100%" border="0">
      ...
      <tr>
        ...
        <div class="ejy-fy">
         <pg:pages  pageNo='${pageNum}' rowCount ="10"  url="queryAction!toQuery.htm" params="signSub=${signSub}&placeF=1&city_id=${placeId}&sort=${sort}&sign=${sign}&filterValue=${filterValue}&eprice=${eprice }&bprice=${bprice }&pageHeadKeywords=${pageHeadKeywords}&orderBySign=${orderBySign }&orderByDesc=${orderByDesc }&sex=${sex}"  total='${game}' ></pg:pages>
        </div>

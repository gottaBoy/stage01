...
$(document).ready(function(){
		<%
			...
			if(userIdStr!=null && Integer.parseInt(userIdStr) > 0){
		%>
				hasSaved = true;
		<%}%>
	getUserPercent();
    	...
    	$("#jcxxForm").validate({
	    	rules: {
	    		"userInfo.nickName":{
	    			required: true,
					rangelength: [2, 25],
					remote:'<%=request.getContextPath()%>/register/registerAction!findUserByNickName.htm?userId=<%=request.getParameter("userId")%>&date='+new Date()
	    		}
			},
	    	messages: {
				"userInfo.nickName": {
...
function getUserInfoGrowth() {
	$.ajax({
		url:"<%=request.getContextPath() %>/member/userinfo!getGrowthList.htm?<%=HttpSessionHelper.getParamData(request)%>",
		type:'POST',
		dataType:'json', 
		success:function(data){
			var d = eval(data);
			...
			for(var i = 0; i < d.length; i++) {
				...
				html += "<a href='javascript:toDelGrowth("+obj.id+");'  class='tianjia'>删除</a></li>";
				html += "<li><hr></li></form>";
			}
...
function addGrowth(){
	...
	$.ajax({
			url:"<%=request.getContextPath() %>/member/growth!saveGrowth.htm?<%=HttpSessionHelper.getParamData(request)%>",
			type:'POST',
...
function submitForm(){
	$.ajax({
		url:"<%=request.getContextPath() %>/member/userinfo!hasUserInfoCost.htm?addUser=true",
		type:'POST',
		success:function(data){
			if(data == "1"){
					if($("#jcxxForm").valid()){
						if(checknull()){
							$.ajax({
								url:"<%=request.getContextPath() %>/member/userinfo!updateOnlyUserInfo.htm?addUser=true&isUpdate=<%=request.getParameter("isUpdate")%>",
								type:'POST',
				...
				$.ajax({
					...
					success:function(data){
						if(data > 0){
							...
							toggleAll(true);
							//$("#myinfobaocun").html('<a href="javascript:modifyForm();">修改</a>');
							window.location = "<%=request.getContextPath()%>/member/orginfo!toAddUser.htm?sign=1&addUser=true&isUpdate=true&userId="+data;
					   }
				   }
				});
			}
...
function toAddArt(sign){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toAddArtInfo.htm?sign='+sign+'&<%=HttpSessionHelper.getParamData(request)%>',{width:750,height:680,resize:false,lock:true});
}
function toUpdateArt(sign,id){
	art.dialog.open('<%=request.getContextPath()%>/member/artInfo!toUpdateArtInfo.htm?id='+id+'&sign='+sign+'&<%=HttpSessionHelper.getParamData(request)%>',{width:550,height:450,lock:true,resize: false});
}
 
			
//修改密码
function updatePass(){
	art.dialog.open('<%=request.getContextPath()%>/user/userAction!doUpdatePassWord.htm?<%=HttpSessionHelper.getParamData(request)%>',{lock:true,resize: false});
}

//实名认证
function realName(){
	art.dialog.open('<%=request.getContextPath()%>/member/userinfo.htm?sign=4&<%=HttpSessionHelper.getParamData(request)%>',{lock:true,resize: false});
}
...
function updateEmailOrMobile(flag){
		art.dialog.open('<%=request.getContextPath()%>/user/userAction!doUpdateEmail.htm?flag='+flag+'&<%=HttpSessionHelper.getParamData(request)%>',{lock:true,resize: false	});
}
...
function showContent(id,price,unit,keywords,profession,costId){
		...
		$.ajax({
				url:"<%=request.getContextPath() %>/member/userinfo!onlySaveCost.htm?addUser=true&<%=HttpSessionHelper.getParamData(request)%>",
				type:'POST',
				data:{
					price:price,
					unit:unit, 
				...
				success:function(data){
					if(data == 1){
						//getUserInfo();
						//location.reload();
						window.location = "<%=request.getContextPath()%>/member/orginfo!toAddUser.htm?sign=1&<%=HttpSessionHelper.getParamData(request)%>";
					}
				}
		});
}
...
function toXiangce(){
	...
	window.location="<%=request.getContextPath() %>/member/artInfo!toArtInfo.htm?sign=1&<%=HttpSessionHelper.getParamData(request)%>";
}
function toShiping(){
	if(hasSaved==false){
		art.dialog.alert("请先保存用户基本信息！");
	...
	window.location="<%=request.getContextPath() %>/member/artInfo!toArtInfo.htm?sign=2&<%=HttpSessionHelper.getParamData(request)%>";
}
...
function toAddCost(index){
	...
	art.dialog.open("<%=request.getContextPath()%>/member/cost!toAddOrUpdateByJs.htm?costId=0&add=true&addUser=true&index="+index,{width:700,height:450,lock:true,resize: false});
}

function toEditCost(index,costId){
	art.dialog.open("<%=request.getContextPath()%>/member/cost!toAddOrUpdateByJs.htm?costId="+costId+"&addUser=true&index="+index,{width:700,height:450,lock:true,resize: false});
}
...
function getUserInfoCost() {
	$.ajax({
		url:"<%=request.getContextPath() %>/member/userinfo!getUserInfoCost.htm?addUser=true&<%=HttpSessionHelper.getParamData(request)%>",
		type:'POST',
		dataType:'json',  
		success:function(data){
			var d = eval(data);
...
<div class="right_tab">
<ul>
<li><a class="change" href='<%=request.getContextPath()%>/member/orginfo!toAddUser.htm?sign=1&<%=HttpSessionHelper.getParamData(request)%>'>基本信息</a></li>
<li><a href="javascript:toXiangce()" id="font2" >相册</a></li>
<li><a href="javascript:toShiping()" id="font3" >视频</a></li>
...
<li>
	...
	<%}else{%>
		<input name="userInfo.mobile" id="userInfo.mobile" value="${userInfo.mobile}"/>
	<%}%>
...
<li>
	...
	<%}else{%>
		<input name="user.email" id="user.email" value="${user.email}"/>
	<%}%>
...
<div ><img src="/stageUpload/headImage/<%=(Integer)HttpSessionHelper.getNewUserIdFromSession()%>.png?date=<%=new java.util.Date()%>"  id="pic001"  /></div>
<a href="javascript:uploadPic('pic001','headImage','<%=(Integer)HttpSessionHelper.getNewUserIdFromSession()%>')"   class="zhezhaoLink">修改头像</a>
</div>
